...
void ShaderQueueSuit_Common::create_pipeline_layout(VkDevice &device) {
    ...
    VkResult result = vkCreateDescriptorSetLayout(device, &descriptor_layout, nullptr,
    ...
    result = vkCreatePipelineLayout(device, &pPipelineLayoutCreateInfo, nullptr,
...
void ShaderQueueSuit_Common::init_descriptor_set(VkDevice &device) {
    ...
    type_count[0].descriptorCount = TextureManager::texNames.size();//描述数量
    ...
    layouts.reserve(TextureManager::texNames.size());
...
void ShaderQueueSuit_Common::create_shader(VkDevice &device) {
    // 1. 加载顶点着色器SPIR-V文件
    std::vector<uint32_t> vertexSpirv = SpirvLoader::loadFromAssets(
            "shaders/commonTexLight.vert.spv"  // 离线编译的顶点着色器SPV
    );
    if (vertexSpirv.empty()) {
        LOGE("zzz 顶点着色器SPV文件加载失败");
        assert(false); // 加载失败时终止，实际项目可更优雅处理
    }

    // 2. 创建顶点着色器模块
    VkShaderModule vertexModule;
    bool success = SpirvLoader::createShaderModule(device, vertexSpirv, vertexModule);
    if (!success) {
        LOGE("zzz 顶点着色器模块创建失败");
        assert(false);
    }
    ...
    shaderStages[0].pName = "main"; // 入口函数名
    shaderStages[0].pSpecializationInfo = nullptr;

    // 4. 加载片段着色器SPIR-V文件
    std::vector<uint32_t> fragmentSpirv = SpirvLoader::loadFromAssets(
            "shaders/commonTexLight.frag.spv"  // 离线编译的片段着色器SPV
    );
    if (fragmentSpirv.empty()) {
        LOGE("zzz 片段着色器SPV文件加载失败");
        vkDestroyShaderModule(device, vertexModule, nullptr); // 清理已创建的资源
        assert(false);
    }

    // 5. 创建片段着色器模块
    VkShaderModule fragmentModule;
    success = SpirvLoader::createShaderModule(device, fragmentSpirv, fragmentModule);
    if (!success) {
        LOGE("zzz 片段着色器模块创建失败");
...
void ShaderQueueSuit_Common::create_pipe_line(VkDevice &device, VkRenderPass &renderPass, VulkanDeviceConfig& deviceConfig) {
    ...
    pipelineCacheInfo.pInitialData = nullptr;//初始数据内容，此处为nullptr
    ...
    result = vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineInfo, nullptr,
                                       &pipeline);//创建管线
