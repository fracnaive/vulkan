...
std::vector<VkFramebuffer> MyVulkanManager::framebuffers;
...
void MyVulkanManager::setWindow(ANativeWindow *window) {
    sWindow = window;
    if (sWindow) {
        screenWidth = ANativeWindow_getWidth(sWindow);
        screenHeight = ANativeWindow_getHeight(sWindow);
...
// 添加设置方法
void MyVulkanManager::setAssetManager(AAssetManager *manager) {
    aAssetManager = manager;
    if (aAssetManager) {
        SpirvLoader::setAAssetManager(aAssetManager);
        LOGE("资源管理器设置成功");
    } else {
        LOGE("资源管理器设置失败：指针为空");
    }
...
void MyVulkanManager::enumerate_vulkan_phy_devices() {
    assert(instance != VK_NULL_HANDLE && "Vulkan实例未初始化");

    gpuCount = 0;//存储物理设备数量的变量
    VkResult result = vkEnumeratePhysicalDevices(instance, &gpuCount, nullptr);//获取物理设备数量
...
void MyVulkanManager::create_vulkan_swapChain() {
    ...
    createInfo.window = sWindow;//给定窗体
    ...
    if (fpCreateAndroidSurfaceKHR == nullptr) {    //判断方法是否加载成功
    ...
    for (uint32_t i = 0; i < queueFamilyCount; i++) {//遍历设备对应的队列家族列表
        ...
        LOGE("队列家族索引=%d %s显示", i, (supportsPresent[i] == 1 ? "支持" : "不支持"));
    }
    ...
    for (uint32_t i = 0; i < queueFamilyCount; ++i)//遍历设备对应的队列家族列表
    {
        if ((queueFamilyprops[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) != 0)//若此队列家族支持图形工作
        {
            ...
            if (supportsPresent[i] == VK_TRUE)//如果当前队列家族支持显示工作
            {
    ...
    if (queuePresentFamilyIndex == UINT32_MAX)//若没有找到同时支持两项工作的队列家族
    {
        ...
        {
            if (supportsPresent[i] == VK_TRUE)//判断是否支持显示工作
            {
                queuePresentFamilyIndex = i;//记录此队列家族索引为支持显示工作的
                break;
            }
    ...
    result = vkGetPhysicalDeviceSurfaceFormatsKHR(gpus[0], surface, &formatCount,
                                                  nullptr);//获取支持的格式数量
    ...
    if (surfCapabilities.currentExtent.width == 0xFFFFFFFF) {
        ...
        LOGE("使用获取的surface能力中的 宽度 %d 高度 %d", swapchainExtent.width,
...
void MyVulkanManager::create_vulkan_DepthBuffer() {
    ...
    mem_alloc.allocationSize = mem_reqs.size;//分配的内存字节数
    ...
    assert(flag);
    LOGE("深度缓冲内存类型索引为%d", mem_alloc.memoryTypeIndex);
    ...
    view_info.image = depthImage;//对应的图像
...
void MyVulkanManager::destroy_vulkan_DepthBuffer() {
    ...
    if (memDepth != VK_NULL_HANDLE) {
        vkFreeMemory(device, memDepth, nullptr);
        memDepth = VK_NULL_HANDLE;
    }
...
//获取设备中支持图形工作的队列, 初始化队列（现在简化，在创建设备时已获取）
...
void MyVulkanManager::create_frame_buffer() {
    ...
    assert(!swapchainImageViews.empty() && "交换链图像视图为空");
...
void MyVulkanManager::flushUniformBuffer()//将当前帧相关数据送入一致变量缓冲
{
    ...
    assert(result == VK_SUCCESS);
    memcpy(pData, fragmentUniformData, sqsCL->bufferByteCount);//将最终矩阵数据拷贝进显存
...
void MyVulkanManager::flushTexToDesSet()//更新绘制用描述集的方法
{
    for (int i=0; i<TextureManager::texNames.size(); i++) {    //遍历所有纹理
        ...
        sqsCL->writes[1].pImageInfo = //写入属性1对应的纹理图像信息
                &(TextureManager::texImageInfoList[TextureManager::texNames[i]]);
        vkUpdateDescriptorSets(device, 2, sqsCL->writes, 0, nullptr);//更新描述集
    }
}
...
void MyVulkanManager::drawObject() {
    ...
    LOGI("zzz drawObject!!!!!!");
    while (loopDrawFlag) {//每循环一次绘制一帧画面
