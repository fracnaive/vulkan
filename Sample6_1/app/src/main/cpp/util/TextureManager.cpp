...
void setImageLayout(VkCommandBuffer cmd, VkImage image,VkImageAspectFlags aspectMask,
    ...
    vkCmdPipelineBarrier(cmd, src_stages, dest_stages, 0, 0, //放置屏障
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu){
    ...
    for (int i = 0; i<SAMPLER_COUNT; i++){ //循环创建指定数量的采样器
        ...
        VkResult result = vkCreateSampler(device,
                                              &samplerCreateInfo, nullptr, &samplerTexture); //创建采样器
...
void TextureManager::destroyTextures(VkDevice& device){ //销毁纹理的方法
    ...
    for(const auto & texName : texNames){ //遍历所有纹理
        vkDestroyImageView(device, viewTextureList[texName], nullptr); //销毁图像视图
        vkDestroyImage(device, textureImageList[texName], nullptr); //销毁图像
        vkFreeMemory(device, textureMemoryList[texName], nullptr); //释放设备内存
    }
}
