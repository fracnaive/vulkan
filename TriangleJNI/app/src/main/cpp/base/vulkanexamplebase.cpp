...
VkDebugUtilsMessengerEXT VulkanExampleBase::debugMessenger = VK_NULL_HANDLE;
...
VkResult VulkanExampleBase::createInstance()
{
    ...
    instanceCreateInfo.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO;//结构体的类型
    instanceCreateInfo.pNext = nullptr;//自定义数据的指针
    instanceCreateInfo.flags = 0;//供将来使用的标志

    std::vector<const char *> instanceExtensionNames;
    const char* validationLayers[] = {"VK_LAYER_KHRONOS_validation"}; //启用验证层
    instanceCreateInfo.enabledLayerCount = 1;//启动的层数量
    instanceCreateInfo.ppEnabledLayerNames = validationLayers;//启动的层名称列表
    ...
    instanceCreateInfo.enabledExtensionCount = instanceExtensionNames.size();//扩展的数量
    instanceCreateInfo.ppEnabledExtensionNames = instanceExtensionNames.data();//扩展名称列表数据
...
VkPipelineShaderStageCreateInfo VulkanExampleBase::loadShader(std::string fileName, VkShaderStageFlagBits stage)
{
	...
	shaderStage.module = vks::tools::loadShader(assetManager, fileName.c_str(), device);
#else
