...
#include <cstring>
#include <array>
#include <vulkan/vulkan.h>  // 包含标准Vulkan头文件
...
void DrawableObjectCommonLight::createVertexBuffer(int dataByteCount, VkDevice &device,
    ...
    if (result != VK_SUCCESS) {
        LOGE("内存分配失败: %d", result);
    }//检查内存分配是否成功

    uint8_t *pData;//CPU访问时的辅助指针
    result = vkMapMemory(device, vertexDataMem, 0, mem_reqs.size, 0,
                         (void **) &pData);//将设备内存映射为CPU可访问
    if (result != VK_SUCCESS) {
        LOGE("内存映射失败: %d", result);
    }//检查映射是否成功
...
DrawableObjectCommonLight::~DrawableObjectCommonLight() noexcept {
    delete[] vdata;  // 使用delete[]释放数组内存
    if (devicePointer) {
...
void DrawableObjectCommonLight::drawSelf(
        VkCommandBuffer &cmd,
